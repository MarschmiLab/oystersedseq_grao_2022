...
## Read data
```{r}
sediment_data <- read.csv("../data/updated_oip copy.csv", stringsAsFactors = TRUE)
...
gnsd_no_tp <-
  sediment_data_new_noTP %>%
  ggplot(mapping = aes(x = Location, y = Normalized.Values)) + 
  geom_point(mapping = aes(color=Treatment, shape = Treatment), size=3 , position = position_jitterdodge()) +
  scale_color_manual(values = c("lightskyblue2", "plum3"))+ 
  geom_boxplot(alpha = 0.3, mapping = aes(color=Treatment), outlier.shape = NA) +
  stat_compare_means(method = "wilcox.test", 
                     pairwise.wilcox.test(sediment_data_new_noTP$Normalized.Values,
                                          sediment_data_new_noTP$Location, p.adjust.method = "bonferroni"), 
                     label.y = 18000) +
  labs(
    x = "Location",
    y = "DNA Concentration (ng DNA / g sediment)",
...
# Marian plot 
sediment_data_new_noTP %>%
  ggplot(aes(x = Location, y = Normalized.Values, 
             color = Treatment, fill = Treatment)) +
  facet_grid(~Location, scales = "free_x")+
  geom_point(size=3 , position = position_jitterdodge(jitter.width = 0.25)) +
  geom_boxplot(alpha = 0.3, outlier.shape = NA) +
  scale_color_manual(values = c("lightskyblue2", "plum3")) +
  scale_fill_manual(values = c("lightskyblue2", "plum3")) +
  labs(
    x = "Location",
    y = "DNA Concentration (ng DNA / g sediment)",
    title = "[DNA] in Oyster and Bare Sediment Samples Across Locations") + 
  # Add the statistical test with an unpaired w
  stat_compare_means(paired = FALSE, method = "wilcox.test",label.y = 11000, 
...
ggsave(filename = "figures/OvsB_location.png", width = 22, height = 14, dpi = 300, units = "cm")
```
...
wilcox_test_df <- read.csv("data/aovOB.csv", stringsAsFactors = TRUE)
